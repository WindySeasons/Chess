...
def on_create_room(data=None):
    ...
    if ai_mode:
        ...
        room['time_red'] = INITIAL_TIME
        room['time_black'] = INITIAL_TIME
        _start_timer(room, code)
        try:
...
def _start_timer(room, code):
    """启动或重启房间倒计时线程"""
    _stop_timer(room)
    stop_event = threading.Event()
    room['timer_stop_event'] = stop_event
    t = threading.Thread(target=_timer_loop, args=(room, code, stop_event), daemon=True)
    t.start()


def _stop_timer(room):
    """停止房间倒计时线程"""
    stop_event = room.get('timer_stop_event')
    if stop_event:
        stop_event.set()
        room['timer_stop_event'] = None


def _timer_loop(room, code, stop_event):
    """每秒递减当前走棋方的倒计时"""
    last_tick = time.time()
    while not room.get('game_over') and room.get('game_started'):
        if stop_event.wait(timeout=1):
            return  # 收到停止信号

        now = time.time()
        elapsed = max(1, int(now - last_tick))
        last_tick = now

        if room.get('game_over') or not room.get('game_started'):
            break

        current_turn = room['current_turn']
        if current_turn:  # True = 红方走棋, 减红方时间
            room['time_red'] -= elapsed
            if room['time_red'] <= 0:
                room['time_red'] = 0
                room['game_over'] = 'black'
